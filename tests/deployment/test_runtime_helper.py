...
class TestGetPsCommand:
    ...
    @pytest.mark.parametrize("all_containers,expected_flags", [
        (False, ['ps', '--format', 'json']),
        (True, ['ps', '-a', '--format', 'json'])
    ])
    @patch('shutil.which')
    @patch('subprocess.run')
...
class TestVerifyRuntimeIsRunning:
    ...
    def test_podman_not_running_macos(self, mock_run, mock_which):
        ...
        mock_run.side_effect = [
            ...
            MagicMock(
                returncode=1,
                stderr="Cannot connect to podman. Is the podman daemon running?"
            )
        ]
